...
export function SuperAdminGrievanceDetailView({
  grievanceId,
}: {
  grievanceId: string;
}) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      await updateGrievance.mutateAsync({ id: grievanceId, status: newStatus as any });
      toast({
        title: "Success",
        description: `Status updated to ${STATUS_CONFIG[newStatus as keyof typeof STATUS_CONFIG]?.label}`,
      });
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const handlePriorityChange = async (newPriority: string) => {
    try {
      await updateGrievance.mutateAsync({ id: grievanceId, priority: newPriority as any });
      toast({
        title: "Success",
        description: `Priority updated to ${PRIORITY_CONFIG[newPriority as keyof typeof PRIORITY_CONFIG]?.label}`,
      });
    } catch (error: any) {
      toast({
        title: "Error",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="border rounded-lg p-4">
            <h3 className="font-semibold mb-4">Manage Grievance</h3>
            <div className="space-y-4">
              <div>
                <Label>Status</Label>
                <Select
            ...
            <div className="space-y-3 text-sm">
              ...
              {grievance.school && (
                <div>
                  ...
                  <span className="ml-2">{grievance.school.name}</span>
                </div>
              )}
            </div>
          </div>
