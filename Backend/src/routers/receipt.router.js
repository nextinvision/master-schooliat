...
router.post(
  ...
  async (req, res) => {
    ...
    if (schoolId && vendorId) {
      return res.status(400).json({ message: "Provide either schoolId or vendorId, not both." });
    }
    if (!schoolId && !vendorId) {
...
router.post(
  ...
  async (req, res) => {
    ...
    // Fetch receipt with school or vendor details
    const receipt = await prisma.receipt.findUniqueOrThrow({
      where: { id },
      include: {
        school: {
    ...
    const paymentMethodDisplay = (receipt.paymentMethod && typeof receipt.paymentMethod === "string"
    ...
    // Prepare bill-to and ship-to: from school or vendor
    let billTo;
    let shipTo;
    if (receipt.vendor) {
      const vendorAddress = receipt.vendor.address && receipt.vendor.address.length
        ? receipt.vendor.address.join("<br>")
        : "N/A";
      billTo = `<strong>${escapeHtml(receipt.vendor.name)}</strong><br>${escapeHtml(vendorAddress)}`;
      shipTo = vendorAddress;
    } else if (receipt.school) {
      const schoolAddress = receipt.school.address && receipt.school.address.length
        ? receipt.school.address.join("<br>")
        : "N/A";
      billTo = `<strong>${escapeHtml(receipt.school.name)}</strong><br>${schoolAddress}`;
      shipTo = schoolAddress;
    } else {
      billTo = "N/A";
      shipTo = "N/A";
    }
